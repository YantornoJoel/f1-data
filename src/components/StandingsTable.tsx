...
export const StandingsTable = ({ f1Standings, f2Standings, f1Drivers, f2Drivers }: StandingsTableProps) => {
  ...
  return <section className="app-section app-section--split" id="clasificacion">
    <div className="standings-copy"><span className="eyebrow">Campeonato de pilotos</span><h2>Tabla de clasificación</h2><p>Alterná F1/F2 y mirá puntos, equipo y foto del piloto. Si no hay foto confiable, usamos iniciales: mejor fallback honesto que asset inventado.</p><div className="championship-toggle"><button type="button" className={series === 'formula1' ? 'is-active' : ''} onClick={() => setSeries('formula1')}>F1</button><button type="button" className={series === 'formula2' ? 'is-active' : ''} onClick={() => setSeries('formula2')}>F2</button></div></div>
