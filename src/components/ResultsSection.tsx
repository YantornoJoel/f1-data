...
const RaceResultCard = ({ race }: { race: EventResult }) => {
  ...
  const defaultType = sessions.find((session) => session.type === 'race')?.type ?? sessions[0]?.type ?? 'race';
  ...
  return <article className="result-card">
    ...
    {selectedSession?.status === 'pending' || selectedSession?.results.length === 0 ? <p className="pending-note">Sesión pendiente. Cuando haya resultado oficial, se carga acá.</p> : <><div className="table-scroll"><table className="results-table"><thead><tr><th>Pos</th><th>Piloto</th><th>Equipo</th><th>Pts</th><th>Tiempo/Dif.</th><th>Vueltas</th></tr></thead><tbody>{visibleResults.map((row) => <tr key={`${race.series}-${race.round}-${selectedSession.type}-${row.driver}`}><td><Position position={row.position} status={row.status} /></td><td>{row.driver}</td><td><span className="team-cell"><i style={{ backgroundColor: getTeamColor(row.team) }} />{row.team}</span></td><td>{isQualifyingSession(selectedSession.type) ? '—' : row.points}</td><td>{row.time || '—'}</td><td>{row.laps ?? '—'}</td></tr>)}</tbody></table></div>{isExpanded && totalPages > 1 ? <div className="results-pagination" aria-label="Paginación de resultados"><button type="button" className="ghost-button" onClick={() => goToPage(safePage - 1)} disabled={safePage === 1}>Anterior</button><span>Página {safePage} de {totalPages}</span><button type="button" className="ghost-button" onClick={() => goToPage(safePage + 1)} disabled={safePage === totalPages}>Siguiente</button></div> : null}{canToggleResults ? <button type="button" className="results-toggle ghost-button" onClick={() => setExpandedMode(!isExpanded)}>{isExpanded ? 'Ver menos' : 'Ver más'}</button> : null}</>}
  </article>;
};
